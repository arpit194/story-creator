...
import type { DialogueLine, Mission, MissionStage, MissionType, StageType } from '#/types/mission'
...
function Collapsible({ label, count, children, defaultOpen = true }: {
  label: string
  count?: number
    ...
    <div>
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: s.muted }}>
            {label}
          </span>
          {count !== undefined && (
            <span className="font-mono-game rounded px-1.5 py-0.5 text-[10px]" style={{ background: 'rgba(255,255,255,0.05)', color: s.muted }}>
              {count}
            </span>
          )}
        </div>
        <ChevronDown
          ...
          style={{ color: s.dim, transform: open ? 'rotate(0deg)' : 'rotate(-90deg)' }}
        />
      </button>
...
function CardCollapsible({ label, count, children, defaultOpen = true }: {
  label: string
  count?: number
  children: React.ReactNode
  defaultOpen?: boolean
...
export function MissionPanel() {
  const { nodes, edges, selectedMissionId, selectMission, updateMission, deleteMission } = useCanvasStore()
  const mission = nodes.find((n) => n.id === selectedMissionId)?.data as Mission | undefined
  ...
  useEffect(() => {
    if (!mission) { setForm(null); loadedIdRef.current = null; return }
  ...
  const prerequisiteNames = edges
    ...
    .flatMap((e) => {
      const name = (nodes.find((n) => n.id === e.source)?.data as Mission | undefined)?.missionName
      return name ? [name] : []
    })
  ...
  const patchStage = (i: number, fields: Partial<MissionStage>) =>
    patch({ stages: form.stages.map((st, idx) => (idx === i ? { ...st, ...fields } : st)) })
  ...
  const addDialogue = (si: number) =>
    patch({
      stages: form.stages.map((st, i) =>
        i === si ? { ...st, dialogues: [...st.dialogues, { id: crypto.randomUUID(), speakerId: '', text: '' }] } : st,
      ),
    })

  const patchDialogue = (si: number, li: number, fields: Partial<DialogueLine>) =>
    patch({
      stages: form.stages.map((st, i) =>
        i === si ? { ...st, dialogues: st.dialogues.map((d, j) => (j === li ? { ...d, ...fields } : d)) } : st,
      ),
    })
  ...
  const removeDialogue = (si: number, li: number) =>
    patch({
      stages: form.stages.map((st, i) =>
        i === si ? { ...st, dialogues: st.dialogues.filter((_, j) => j !== li) } : st,
      ),
    })
  ...
  return (
    <>
      <div
        ...
        style={{ width: 420, background: 'linear-gradient(180deg, #0d1017, #080a0e)', borderLeft: '1px solid var(--border)' }}
      >
        {/* Header */}
        <div className="flex shrink-0 items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
          <div className="flex items-center gap-3">
            <div className="h-2 w-2 rounded-full" style={{ background: accent, boxShadow: `0 0 8px ${accent}` }} />
            <span className="font-mono-game text-xs font-semibold uppercase tracking-widest" style={{ color: s.muted }}>Mission</span>
          </div>
        ...
        <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-5">
          <div className="flex flex-col gap-3">
            <Field label="Mission Name">
              <input className={input} value={form.missionName} onChange={(e) => patch({ missionName: e.target.value })} />
            </Field>
          ...
          <Collapsible label="Prerequisites" count={prerequisiteNames.length}>
            {prerequisiteNames.length === 0 ? (
              <p className="text-xs italic" style={{ color: s.dim }}>None — connect edges on the canvas</p>
            ) : (
              <div className="flex flex-col gap-1.5">
                {prerequisiteNames.map((name) => (
                  <div key={name} className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)' }}>
                    <span className="h-1 w-1 shrink-0 rounded-full" style={{ background: '#334155' }} />
                    <span className="font-mono-game text-xs" style={{ color: '#94a3b8' }}>{name}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: s.muted }}>Stages</span>
                <span className="font-mono-game rounded px-1.5 py-0.5 text-[10px]" style={{ background: 'rgba(255,255,255,0.05)', color: s.muted }}>
                  {form.stages.length}
                </span>
              </div>
            ...
            {form.stages.length === 0 && (
              <div className="flex items-center justify-center rounded-xl py-8 text-xs" style={{ border: '1px dashed var(--border-bright)', color: s.dim }}>
                No stages yet
              </div>
            )}
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          onConfirm={() => { deleteMission(form.id); selectMission(null) }}
...
function StageCard({ stage, index, characters, buildings, onPatch, onRemove, onAddDialogue, onPatchDialogue, onRemoveDialogue }: StageCardProps) {
  const stageColor = STAGE_COLOR[stage.stageType] ?? '#64748b'
  const [confirmRemove, setConfirmRemove] = useState(false)
  const [confirmRemoveLine, setConfirmRemoveLine] = useState<number | null>(null)

  return (
    <>
      <div className="overflow-hidden rounded-xl" style={{ background: 'var(--surface-2)', border: '1px solid var(--border-bright)' }}>
        <CardCollapsible label={`Stage ${index + 1} · ${stage.stageType}`} defaultOpen={true}>
          <div className="flex flex-col gap-3">
            {/* Type pills + delete */}
            ...
            <Field label="Target Building">
              <select className={select} value={stage.targetBuildingId ?? ''} onChange={(e) => onPatch({ targetBuildingId: e.target.value || null })}>
            ...
            <Field label="Status Message">
              <input className={input} placeholder="Shown in HUD during this stage" value={stage.statusMessage} onChange={(e) => onPatch({ statusMessage: e.target.value })} />
            </Field>

            {stage.stageType === 'Pickup' && (
              <Field label="Cargo Weight">
                <input type="number" min={0} className={input} value={stage.cargoWeight} onChange={(e) => onPatch({ cargoWeight: Number(e.target.value) })} />
              </Field>
            )}

            {stage.stageType === 'Drop' && (
              <Field label="Time Limit (seconds)">
                <input type="number" min={0} className={input} placeholder="0 = no limit" value={stage.timeLimit} onChange={(e) => onPatch({ timeLimit: Number(e.target.value) })} />
              </Field>
            )}

            {/* Dialogue nested collapsible — uses same CardCollapsible style */}
            <div className="-mx-4 overflow-hidden rounded-lg" style={{ background: 'var(--surface-3)', border: '1px solid var(--border)' }}>
              <CardCollapsible label="Dialogue Lines" count={stage.dialogues.length} defaultOpen={false}>
                <div className="flex flex-col gap-2">
                  {stage.dialogues.map((line, li) => (
                    <div key={line.id} className="flex flex-col gap-2 rounded-lg p-3" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
                      <div className="flex items-center gap-2">
                        <select
                          className={`${select} flex-1`}
                          value={line.speakerId}
                          onChange={(e) => onPatchDialogue(li, { speakerId: e.target.value })}
                        >
                      ...
                      <textarea
                        ...
                        value={line.text}
                        onChange={(e) => onPatchDialogue(li, { text: e.target.value })}
                      />
                    </div>
                  ))}
                  <button
                    ...
                    style={{ color: stageColor, border: `1px dashed ${stageColor}30` }}
                  >
                    <Plus className="h-3 w-3" />
                    Add Line
                  </button>
      ...
      {confirmRemoveLine !== null && (
        <ConfirmDialog
          ...
          onConfirm={() => { onRemoveDialogue(confirmRemoveLine); setConfirmRemoveLine(null) }}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      <span className="text-[11px] font-medium uppercase tracking-wider" style={{ color: '#475569' }}>{label}</span>
      {children}
    </div>
  )
}
...
const inputBase = 'w-full rounded-lg px-3 py-2 text-sm text-white placeholder-[#334155] focus:outline-none transition-colors duration-150'
